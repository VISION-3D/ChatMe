...
app.post('/api/chatrooms', auth, (req, res) => {
  ...
  try {
    ...
    console.log('🔍 DEBUG - User qui fait la requête:');
    console.log('  - ID:', req.user.id);
    console.log('  - Username:', req.user.username);
    
    console.log('🔍 DEBUG - Tous les salons existants:');
    chatRooms.forEach(room => {
      console.log(`  - Salon: "${room.name}"`);
      console.log('    Membres:', room.members.map(m => `${m.username} (ID:${m._id})`));
      console.log('    Créé par:', room.createdBy.username);
    });
    ...
    console.log(`✅ Salons trouvés pour ${req.user.username}: ${userRooms.length} salons`);
